...
async def signup(body: UserModel, db: Session = Depends(get_db)):
    ...
    if exist_user:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="HTTP 409 Conflict")
    new_user = Contacts(email=body.username, password=hash_handler.get_password_hash(body.password))
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
